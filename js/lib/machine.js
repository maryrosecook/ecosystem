...
var Machine = Base.extend({
  ...
  read: function(subTreeJson, parent, actor) {
    ...
    if(subTreeJson.pointer == true)
      node = new Pointer(subTreeJson.id, subTreeJson.test, subTreeJson.strategy, parent, actor);
    else
      node = new State(subTreeJson.id, subTreeJson.test, subTreeJson.strategy, parent, actor);
...
var Node = Base.extend({
  ...
  constructor: function(id, test, strategy, parent, actor) {
    this.id = id;
    this.test = test;
  ...
  can: function() {
    ...
    if(functionName === undefined) // no override so go with default function name
      functionName = "can" + this.id[0].toUpperCase() + this.id.substring(1, this.id.length);
  ...
  nearestNamesakeAncestor: function(id) {
    if(this.parent === null)
      return null;
    else if(this.parent.id == id)
      return this.parent;
    else
      return this.parent.nearestNamesakeAncestor(id);
  },
}, {
...
var State = Node.extend({
  ...
  run: function() {
    this.actor[this.id].call(this.actor); // run the action
  },
}, {
